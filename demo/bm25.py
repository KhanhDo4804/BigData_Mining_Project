...
class FastBM25Okapi:
    def __init__(self, docs, k1=1.5, b=0.75):
        self.k1, self.b, self.n_docs = k1, b, len(docs)
        ...
        for term_id, (term, hits) in enumerate(postings.items()):
            ...
            for doc_id, tf in hits:
                norm = 1 - b + b * self.doc_len[doc_id] / self.avgdl if self.avgdl else 1
                rows.append(doc_id)
                cols.append(term_id)
...
def make_submission(test_file="de_thi.json", corpus_file="dataset.json", output_file="submission.json"):
